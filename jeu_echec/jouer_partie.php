...
<body>
	<?php
		...
		echo '<p id = couleur_du_joueur>Vous êtes la couleur ' . $couleur_joueur . '<br></p>';
		...
		if($statut_partie == "finie") /* si la partie est finie */ {
			...
			echo '
			<form method="post">
				<label for="reponse_null">l\'adversaire a proposé de faire égalité (partie nulle ou pat), voulez vous accepter ou refuser ?</label>
			...
			if (isset($_POST['reponse_null']) && $_POST['reponse_null'] == "accepter") {
				...
				$bdd->query($req);
				echo "vous avez accepté l\'égalité";
				echo'<form action="main.php" method="post">
			...
			if ((intdiv($proposition_mat, 10) == 1 && $couleur_joueur == "blanc") || (intdiv($proposition_mat, 10) == 2 && $couleur_joueur == "noir")) /* c'est le joueur connecté qui a fait la proposition de mat ou de refus -> on ne fait rien */ {
				echo 'C\'est noté.<br>
				Dans l\'attente de la réponse de votre adversaire, veuillez actualiser régulièrement la page.';
			...
			} else if ($proposition_mat%10 == 3) /* mat refusé */ {
				$req = "UPDATE Partie SET proposition_mat = NULL WHERE id_partie = '$id_partie'";
				$bdd->query($req);
				echo 'mat refusé';
				echo'<form action="main.php" method="post">
					<button type="submit">veuillez retourner au menu principale</button>
				</form>';
			}
		} else /* l'adversaire n'a pas proposé nulle ni mat in pat (execution normale) */ {
			// quelques fonctions :
			...
			if(($joueur_dont_c_est_le_tour == 1 && $couleur_joueur == "blanc") || ($joueur_dont_c_est_le_tour == 2 && $couleur_joueur == "noir"))/* si c'est a moi de jouer pendant un tour normal */ {
				...
				if (isset($_POST["lettre"]) && isset($_POST["nombre"]) && isset($_POST["lettre2"]) && isset($_POST["nombre2"])) /* si l'utilisateur a soumit le coup a jouer */{
					...
					else
					{
						...
						if(appartient($tableau_mvts_disponibles, $coordonnees2)) /* le joueur a joué un coup valide */ {
							// redéclaration locale de la variable $pv, car elle a seulement été déclaree dans la fonction qui declare le nouvel échiquier
							$pv = array('pv', 'pv', '');

							// points
							if($couleur_joueur == "blanc") /* enregistrement des points potentiellement gagnée */ {
								if ($echiquier[$ordonnee2][$abscisse2][0] == 'pion') {
									$req = "UPDATE Partie SET point_1 = (SELECT point_1 FROM Partie WHERE id_partie = '$id_partie') + 1 WHERE id_partie = '$id_partie'";
									$bdd->query($req);
								} else if ($echiquier[$ordonnee2][$abscisse2][0] == 'tour') {
									$req = "UPDATE Partie SET point_1 = (SELECT point_1 FROM Partie WHERE id_partie = '$id_partie') + 5 WHERE id_partie = '$id_partie'";
									$bdd->query($req);
								} else if ($echiquier[$ordonnee2][$abscisse2][0] == 'cavalier') {
									$req = "UPDATE Partie SET point_1 = (SELECT point_1 FROM Partie WHERE id_partie = '$id_partie') + 3 WHERE id_partie = '$id_partie'";
									$bdd->query($req);
								} else if ($echiquier[$ordonnee2][$abscisse2][0] == 'fou') {
									$req = "UPDATE Partie SET point_1 = (SELECT point_1 FROM Partie WHERE id_partie = '$id_partie') + 3 WHERE id_partie = '$id_partie'";
									$bdd->query($req);
								} else if ($echiquier[$ordonnee2][$abscisse2][0] == 'reine') {
									$req = "UPDATE Partie SET point_1 = (SELECT point_1 FROM Partie WHERE id_partie = '$id_partie') + 9 WHERE id_partie = '$id_partie'";
									$bdd->query($req);
								}
							} else if ($couleur_joueur == "noir") /* enregistrement des points potentiellement gagnée */ {
								if ($echiquier[$ordonnee2][$abscisse2][0] == 'pion') {
									$req = "UPDATE Partie SET point_2 = (SELECT point_2 FROM Partie WHERE id_partie = '$id_partie') + 1 WHERE id_partie = '$id_partie'";
									$bdd->query($req);
								} else if ($echiquier[$ordonnee2][$abscisse2][0] == 'tour') {
									$req = "UPDATE Partie SET point_2 = (SELECT point_2 FROM Partie WHERE id_partie = '$id_partie') + 5 WHERE id_partie = '$id_partie'";
									$bdd->query($req);
								} else if ($echiquier[$ordonnee2][$abscisse2][0] == 'cavalier') {
									$req = "UPDATE Partie SET point_2 = (SELECT point_2 FROM Partie WHERE id_partie = '$id_partie') + 3 WHERE id_partie = '$id_partie'";
									$bdd->query($req);
								} else if ($echiquier[$ordonnee2][$abscisse2][0] == 'fou') {
									$req = "UPDATE Partie SET point_2 = (SELECT point_2 FROM Partie WHERE id_partie = '$id_partie') + 3 WHERE id_partie = '$id_partie'";
									$bdd->query($req);
								} else if ($echiquier[$ordonnee2][$abscisse2][0] == 'reine') {
									$req = "UPDATE Partie SET point_2 = (SELECT point_2 FROM Partie WHERE id_partie = '$id_partie') + 9 WHERE id_partie = '$id_partie'";
									$bdd->query($req);
								}
							}

							// enregistrer le coup joué en bdd
							$nd = $_POST['nombre'];
							$ld = $_POST['lettre'];
							$na = $_POST['nombre2'];
							$la = $_POST['lettre2'];
							$req = "INSERT INTO coup (id_partie, id_coup_dans_la_partie, lettre_depart, chiffre_depart, lettre_arrivee, chiffre_arrivee)
							SELECT @id_partie := '$id_partie',
								IFNULL((SELECT MAX(id_coup_dans_la_partie) FROM coup WHERE id_partie = @id_partie), 0) + 1,
								'$ld',
								'$nd',
								'$la',
								'$na'";
						...
						else
						{
							...
							echo'
								<form method="post">
									<input type="submit" name="reload_page" value="D\'accord">
								</form>';
				...
				echo'
				<form method="post" id="formulaire_proposition_nulle_pat">
					<h2>Proposer partie nulle ou un pat</h2>
					<input type = "submit" name = "proposer_partie_nulle_pat" value = "Proposer partie nulle ou pat">
				</form>';

				if(isset($_POST['proposer_partie_nulle_pat']) && $_POST['proposer_partie_nulle_pat'] == "Proposer partie nulle ou pat") /* si le joueur n'a pas soumi de coup ET qu'il veut proposer partie nulle ou pat*/ {
					if($couleur_joueur == "blanc") {
					...
					// desactiver l'id avec javascript formulaire_proposition_nulle_pat
					echo '<script>document.getElementById("formulaire_proposition_nulle_pat").remove(); </script>';
					echo '<script>document.getElementById("formulaire_de_demande_de_piece_a_deplacer").remove(); </script>';
					
					echo "vous avec bien proposé partie nulle ou pat, vous devez maintenant retourner sur le menu principale puis revenire sur la partie";
		...
		$req = "SELECT id_coup_dans_la_partie, chiffre_depart, lettre_arrivee, chiffre_arrivee, lettre_depart 
		FROM coup
		WHERE id_partie = '$id_partie' ORDER BY id_coup_dans_la_partie";
		
		$parties = $bdd->query($req);
		...
		while ($row = $parties->fetch()) 
		{
			...
			echo '<td>' . $row['id_coup_dans_la_partie'] . '</td>';
			echo '<td>' . $row['lettre_depart'] . $row['chiffre_depart'] . '</td>';
			echo '<td>' . $row['lettre_arrivee'] . $row['chiffre_arrivee'] . '</td>';
			if ($row['id_coup_dans_la_partie']%2 == 0){
				echo '<td>' . "Noir" . '</td>';
			}
			else{
				echo '<td>' . "Blanc" . '</td>';
